...
(function (window, angular, undefined) {
     ...
     */
                ...
                $timeout(function () {
                    ...
                    return openPopup.then(function (oauthData) {
                        ...
                        if (oauthData.state && oauthData.state !== storage.get(stateName)) {
                            return defer.reject(
                                'The value returned in the state parameter does not match the state value from your original ' + 'authorization code request.'
                            );
                        }
        ...
        return function () {
            ...
            Oauth1.open = function (options, userData) {
                ...
                if (window.cordova) {
                    popupWindow = popup.open(serverUrl, defaults.name, defaults.popupOptions);
                } else {
                    popupWindow = popup.open(serverUrl, defaults.name, defaults.popupOptions);
                }
    ...
    }]).factory('SatellizerPopup', ['$q', '$interval', '$window', 'SatellizerConfig', 'SatellizerUtils', '$localstorage', '$http', function ($q, $interval, $window, config, utils, $localstorage, $http) {
        var Popup = {};
        ...
        Popup.eventListener = function (redirectUri) {
            ...
            Popup.popupWindow.addEventListener('loadstart', function (event) {
                ...
                if (window.cordova && cookieMaster) {
                    cookieMaster.getCookieValue('http://ionic.rovrapp.com', 'connect.sid', function (data) {
                        if (!data.cookieValue) return;
                        $localstorage.set('isAuthenticated', true);
                        $localstorage.set('connect.sid', data.cookieValue);
                        $http.defaults.headers.common['connect.sid'] = $localstorage.get('connect.sid');
                        $http({ method: 'GET', url: 'http://ionic.rovrapp.com/users/me', }).then(function (response) {
